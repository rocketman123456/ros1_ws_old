...
public:
    ...
    Point(const Vector &position, const Eigen::Vector3f &color, float intensity, const Vector &normal, float normalConfidence, float curvature)
        : mPosition(position), mColor(color), mIntensity(intensity), mNormal(normal), mInvNormal(1 / mNormal.array()), mNormalConfidence(normalConfidence), mCurvature(curvature), mAngle((std::atan2(mNormal.y(), mNormal.x()) + M_PI) * 180.0f / M_PI)
    {
